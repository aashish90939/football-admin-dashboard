...
const Team = () => {
  ...
  const generalMembers = players.filter((p) => p.membership_type === "general");
  const playerMembers = players.filter((p) => p.membership_type === "player");
  const honoraryMembers = players.filter(
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-6 mb-12">
      {members.map((player, index) => (
        ...
        >
          ...
          />
          <h3 className="text-md font-semibold text-blue-900">{player.name}</h3>
          <p className="text-gray-700 text-xs capitalize mt-1">{player.role}</p>
